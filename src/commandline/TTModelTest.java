...
class TTModelTest {
	...
	static void setUpBeforeClass()  {
		System.out.print("Please simulate the player's choice of the number of AIs by entering a number between 1 and 4:");
		testModel = new TTModel();
	}
	...
	public void testCardsRead() {
		...
		assertEquals(6 , testModel.getAttributeNames().length, "Check that method reads attributes and stores them into array of size 5");
	...
	public void testCardsDistributed() {
		...
		if(numberOfPlayers == 3) {
			...
			for(Player p : players) {
				assertTrue(p.getPlayerCards().size() == 40/numberOfPlayers,"otherwise each players gets 40/total players cards");
			}
		}
	...
	public void testIndexOfHumanPlayer() {
		assertEquals("You",testModel.getPlayers().get(testModel.getIndexOfHumanPlayer()).getPlayerName(),"check that index points to human player");
	}
	
	@Test
	public void testNumberOfAIsChoice() {
		assertEquals(testModel.getAIPlayerNumber(),testModel.getPlayers().size()-1,"test that the number of players in the game corresponds to the player's choice");
	}
}
